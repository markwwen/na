...
| 历史与上下文 | [history.ts](src/history.ts) 校验完整轮次；[session.ts](src/session.ts) 保存与恢复；[context.ts](src/context.ts) 生成摘要和请求上下文 |
...
- **归档与请求分离：** 摘要只覆盖完整的早期轮次，完整历史仍保留。旧轮次的 thinking 可以在发送请求时过滤；当前工具调用链的 thinking 与签名保持完整，不修改归档原文。上下文预算是字符估算，不是精确 token 计数。
